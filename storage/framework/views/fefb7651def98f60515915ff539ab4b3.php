...
<div class="min-h-screen bg-red-800 pb-32" x-data="{ 
    ...
    <div class="w-full max-w-xl sm:max-w-2xl lg:max-w-6xl mx-auto px-3 sm:px-4">
        <form action="<?php echo e(route('orders.store')); ?>" method="POST" class="space-y-6 sm:space-y-8" id="orderForm">
            <?php echo csrf_field(); ?>
            ...
            <!-- Topping Categories -->
            <div class="w-full max-w-xl sm:max-w-2xl lg:max-w-6xl mx-auto px-3 sm:px-4">
                <?php $__currentLoopData = $toppingCategories; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $category): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <div class="space-y-3 sm:space-y-4 mb-16">
                    <h3 class="text-center font-['Kalam'] text-xl sm:text-2xl text-white mb-6"><?php echo e($category->name); ?> :</h3>
                    
                    <div class="relative mx-auto">
                        <?php if(count($category->toppings) > 4): ?>
                        ...
                        <div class="carousel-items flex gap-3 sm:gap-4 overflow-x-auto snap-x snap-mandatory scrollbar-hide px-8 sm:px-12 
                            <?php if(count($category->toppings) <= 4): ?> lg:justify-center <?php endif; ?>">
                            <?php $__currentLoopData = $category->toppings; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $topping): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <div class="flex-none w-40 sm:w-56 snap-start" data-topping-id="<?php echo e($topping->id); ?>">
                                <div class="relative bg-[#FFE5E5] rounded-2xl overflow-hidden shadow-lg">
                                    ...
                                    <img src="<?php echo e(asset('storage/images/toppings/' . $topping->image)); ?>" 
     class="w-full h-full object-cover <?php echo e((!$topping->is_available || $topping->stock <= 0) ? 'opacity-50' : ''); ?>" 
     alt="<?php echo e($topping->name); ?>">
                                        
                                        <!-- Unavailable Overlay -->
                                        <?php if(!$topping->is_available || $topping->stock <= 0): ?>
                                        ...
                                        <div class="absolute top-2 right-2 z-10">
                                            <div class="w-5 h-5 sm:w-6 sm:h-6 rounded-full bg-white transition-colors duration-200 peer-checked:bg-[#90B77D] flex items-center justify-center 
                                                <?php echo e((!$topping->is_available || $topping->stock <= 0) ? 'opacity-50' : ''); ?>">
                                    ...
                                    <div class="p-3 sm:p-4">
                                        <h4 class="font-['Kalam'] text-lg sm:text-xl text-center <?php echo e((!$topping->is_available || $topping->stock <= 0) ? 'opacity-50' : ''); ?>">
                                            <?php echo e($topping->name); ?>

                                        </h4>
                                        <p class="text-center text-sm sm:text-base bg-[#90B77D]/20 rounded-full py-1 <?php echo e((!$topping->is_available || $topping->stock <= 0) ? 'opacity-50' : ''); ?>" 
                                           style="font-family: 'Courgette', cursive;" 
                                           data-price="<?php echo e($topping->price); ?>">
                                            Rp <?php echo e(number_format($topping->price)); ?>
                                    ...
                                    <!-- Checkbox -->
                                    <input type="checkbox" 
                                           id="topping-<?php echo e($topping->id); ?>" 
                                           name="toppings[]" 
                                           value="<?php echo e($topping->id); ?>" 
                                           class="peer hidden"
                                           <?php echo e((!$topping->is_available || $topping->stock <= 0) ? 'disabled' : ''); ?>>

                                    <!-- Label -->
                                    <label for="topping-<?php echo e($topping->id); ?>" 
                                           class="absolute inset-0 cursor-pointer z-20 <?php echo e((!$topping->is_available || $topping->stock <= 0) ? 'cursor-not-allowed' : ''); ?>">
                                    </label>
                                </div>
                            </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Icons
    const chiliIcon = `
        <img src="<?php echo e(asset('images/chilli.png')); ?>" class="h-8 w-8" alt="chili">
    `;
    const bowlIcon = `
        <img src="<?php echo e(asset('images/bowl.png')); ?>" class="h-8 w-8" alt="bowl">
    `;
    ...
    function updateCookingIcons(type) {
        ...
        switch(type) {
            case 'kering':
                bowlImage = `<img src="<?php echo e(asset('images/bowl 2.png')); ?>" class="h-8 w-8" alt="bowl-kering">`;
                break;
            case 'nyemek':
                bowlImage = `<img src="<?php echo e(asset('images/bowl 1.png')); ?>" class="h-8 w-8" alt="bowl-nyemek">`;
                break;
            case 'kuah':
                bowlImage = `<img src="<?php echo e(asset('images/bowl.png')); ?>" class="h-8 w-8" alt="bowl-kuah">`;
                break;
        }
...
<?php echo $__env->make('layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\Users\mira3\Downloads\seblakFINALLY\resources\views/menu/index.blade.php ENDPATH**/ ?>